...
use Yii;
use jayfir\pdd\TopLogger;
use jayfir\pdd\ResultSet;
use yii\base\Component;

class PopClient extends Component
{

    public $appkey;
    public $secretKey;
    public $gatewayUrl = "http://gw-api.pinduoduo.com/api/router";
    public $format = "XML"; //XML或者JSON（二选一），默认JSON，注意是大写。
    ...
    protected $sdkVersion = "pop-sdk-php-20180504";
    ...
    public function curl($url, $postFields = null)
    {
        ...
        curl_setopt($ch, CURLOPT_USERAGENT, "pop-sdk-php");
        //https 请求
    ...
    protected function logCommunicationError($apiName, $requestUrl, $errorCode, $responseTxt)
    {
        ...
        $logger->conf["log_file"] = Yii::$app->getRuntimePath() . "/pdd/logs/pop_comm_err_" . $this->appkey . "_" . date("Y-m-d") . ".log";
     ...
     * @param type $access_token 需要授权
     * @param type $bestUrl
     * @return type
     */
    public function execute($request, $access_token = null, $bestUrl = null)
    {
        ...
        $sysParams["data_type"] = $this->format;
        $sysParams["sign_method"] = $this->signMethod;
        $sysParams["type"] = $request->getApiMethodName();
        $sysParams["timestamp"] = time(); //时间戳
        if (null != $access_token) {
            $sysParams["access_token"] = $access_token; //需要授权
        }
        $apiParams = array();
        //获取业务参数
...
//            $sysParams["partner_id"] = $this->sdkVersion;
        ...
        if (isset($respObject->code)) {
            $logger = new TopLogger();
            $logger->conf["log_file"] = Yii::$app->getRuntimePath() . "/pdd/logs/pop_biz_err_" . $this->appkey . "_" . date("Y-m-d") . ".log";
            $logger->log(array(
                date("Y-m-d H:i:s"),
                $resp
            ));
    ...
    {
        if (!isset($paramsArray["type"])) {
            trigger_error("No api name passed");
        }
        $inflector = new LtInflector;
        $inflector->conf["separator"] = ".";
        $requestClassName = ucfirst($inflector->camelize(substr($paramsArray["type"], 7))) . "Request";
        if (!class_exists($requestClassName)) {
            trigger_error("No such api: " . $paramsArray["type"]);
        }

        $access_token = isset($paramsArray["access_token"]) ? $paramsArray["access_token"] : null;
        ...
        return $this->execute($req, $access_token);
    }
